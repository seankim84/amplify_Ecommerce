  ...
  componentDidMount(){
    ...
    .subscribe({
      next: productData => {
        ...
        const prevProduct  = this.state.market.products.items.filter(item => item.id !== createdProduct.id
        )
        const updatedProducts = [createdProduct, ...prevProduct]
    ...
    .subscribe({
      next: productData => {
        ...
        const updatedProducts  = this.state.market.products.items.filter(item => item.id !== deletedProduct.id
        )
        const market = {...this.state.market}
        market.products.items = updatedProducts;
        this.setState({ market })
    ...
    return isLoading ? (
      ...
      <React.Fragment>
        <Link className="link" to="/">
          Back to the Markets List
        </Link>

        <span className="items-center pt-2">
          <h2 className="mb-mr">{market.name} - {market.owner}</h2>
        </span>
        <span className="items-center pt-2">
